...
RUN Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12; Invoke-Expression ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))

# Add Chocolatey to PATH
ENV PATH="C:\\ProgramData\\chocolatey\\bin;${PATH}"

# Install Windows-native CLI & Development Tools
RUN choco install -y git nodejs-lts python openjdk golang rust dotnet-sdk vscode 7zip winrar curl wget unzip cmake visualstudio2022buildtools --no-progress

# Install Windows-native Debugging Tools
RUN choco install -y sysinternals windbg processhacker --no-progress
